...
- **Native ESM only, no CommonJS.** Hand-written `require()` is a code smell. Everything compiles to native ESM (`"type": "module"`, tsconfig `module: nodenext`); source imports name local files by their real extension (`./foo.ts`), and `rewriteRelativeImportExtensions` rewrites those to `.js` in emitted output so Node still runs plain ESM files on disk. The single sanctioned exception is the preload script (Electron's sandbox requires a CommonJS preload, so its source carries the `.cts` extension and tsc emits it as `.cjs`). Prefer removing the cause of a wart over documenting the wart.
- **No hand-written `.d.ts` files.** Shared types live in ordinary `.ts` modules using `export type` / `import type`, so every name has a greppable import stating where it comes from. `declare global` is not used at all (see the rule above): the page's globals are read where they are consumed, `window.bridge` once in the renderer's own bridge module and imported from there. An empty emitted `.js` for a types-only module is acceptable.
