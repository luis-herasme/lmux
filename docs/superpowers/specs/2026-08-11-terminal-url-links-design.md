...
All changes live in the terminal link provider
(`src/renderer/tabs/file-links.ts`); nothing changes in the main process.
...
**Precedence.** URL matches are collected first. A file-path match whose range
falls inside a URL's range is discarded, so `https://a.com/app.js` is one URL
link and never a file link. File paths outside URLs behave exactly as today.
...
**Structure.** The buffer-coordinate math (match index → 1-based x/y range)
is currently inline in the file-link loop and is needed by both patterns, so
it moves to a shared helper in the same file. Since the module now recognizes
more than file paths, it is renamed `file-links.ts` → `links.ts` and
`registerFileLinks` → `registerTerminalLinks`.
